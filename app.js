...
function markerNode(emoji,category,extra=''){const el=document.createElement('div');const kind=category.includes(',')?'multi':category;el.className=`marker marker-${kind} ${extra}`;el.innerHTML=`<span>${emoji}</span>`;return el;}
...
async function main(){
  ...
  for(const place of places.items){
    const marker=new google.maps.marker.AdvancedMarkerElement({map,position:{lat:place.lat,lng:place.lng},title:place.name,content:markerNode(emojiFor(place.category),place.category)});
  ...
  function locate(){
    ...
    navigator.geolocation.getCurrentPosition(pos=>{const here={lat:pos.coords.latitude,lng:pos.coords.longitude};new google.maps.marker.AdvancedMarkerElement({map,position:here,title:'My location',content:markerNode('📍','me','me'),zIndex:100});bounds.extend(here);map.fitBounds(bounds,70);statusEl.textContent=`${places.items.length} recommendations · location shown`;},()=>{statusEl.textContent='Location permission was not granted';},{enableHighAccuracy:true,timeout:12000,maximumAge:60000});
  }
